async function renderUni() {
    ...
    for(var i=1;i<lungime;i++){
        ...
        if(document.getElementById("darkTheme").innerHTML ==="1"){
            document.querySelectorAll('[universitate]')[i].getElementsByClassName("more")[0].innerHTML =  "<a href='http://"+ universityDetails.link +"' target='_blank' class='btn buton-link dark-btn'>Viziteaza website</a><a href='/universitati/"+ universityDetails.nume +"|"+  universityDetails._id + "' class='btn buton-link detalii dash-hide dark-btn'>Mai multe detalii</a>";
        }
        else{
